...
class IFNet(nn.Module):
    def __init__(self, scale=1):
        ...
        self.block1 = IFBlock(9 + 48 + 8 + 4 + 8 + 32, c=128)
        self.block2 = IFBlock(9 + 48 + 8 + 4 + 8 + 32, c=96)
        self.encode = Head()
        self.scale_list = [x / scale for x in [16, 8, 4]]
    ...
    def forward(self, img0, img1, img2, timestep):
        ...
        for scale_idx in range(0, len(self.scale_list)):
            ...
            if flow is None:
                flow, mask, feat = block[scale_idx](torch.cat((img0_s[:, :3], img1_s[:, :3], img2_s[:, :3],
                                                               f0_s, f1_s, f2_s,
                                                               img0_s[:, :1].clone() * 0 + timestep
                                                               ), 1))
            else:
                ...
                if current_scale <= 1:
                    ...
                    flow, mask, feat = self.batch_interpolate(flow, mask, feat, scale_factor=current_scale)
                    flow *= current_scale

                flow, mask, feat = block[scale_idx](
                    torch.cat((img0_s[:, :3], img1_s[:, :3], img2_s[:, :3], f0_s, f1_s, f2_s,
            ...
            if flow_scale != 1:
                flow, mask, feat = self.batch_interpolate(flow, mask, feat, scale_factor=flow_scale)
                flow *= flow_scale
